...
def train_model(df):
    ...
    cat = ["category"]
    X = df[num+cat]; y = df["google_rating"]
...
# Price 1–10
pmin,pmax = 1,10
price = st.sidebar.slider("Price level", pmin, pmax, (pmin,pmax))
...
c2.metric("Avg Rating", f"{df['predicted_rating'].mean():.2f}")
...
# ───── KEYWORD EXTRACTION ─────
text = r["combined_reviews_clean"] or ""
# simple: split on nonwords, count >3 letters
words = re.findall(r"\b[a-zA-Z]{4,}\b", text.lower())
common = [w for w,_ in Counter(words).most_common(8)]
st.subheader(f"{sel_name} — Pred: {r['predicted_rating']} ⭐")
...
if {"latitude","longitude"}.issubset(r.index):
    ...
    clr = [int(255*(1-(r['predicted_rating']-1)/4)),
           int(120+135*(r['predicted_rating']-1)/4),200,180]
    layer = pdk.Layer("ScatterplotLayer", data=pd.DataFrame([r]), get_position='[longitude,latitude]',
                      get_fill_color=clr, get_radius=100, pickable=True)
    st.pydeck_chart(pdk.Deck(initial_view_state=view, layers=[layer]))
else:
    st.error("No location data")

# ───── SHOW FULL REVIEWS ─────
st.subheader("Customer Reviews")
for i, snippet in enumerate(text.split("||")):
