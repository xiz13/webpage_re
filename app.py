...
def train_model(df):
    ...
    cat_feats = ["category"]
    X = df[num_feats + cat_feats]; y = df["google_rating"]
    ...
    # Define regressors with keyword arguments only
    models = {
    ...
    for pipe_name, model in models.items():
    ...
    # Final fit and attach predictions
    best_pipe.fit(X, y)
...
# ───── KEYWORDS ─────
text = r.get("combined_reviews_clean", "")
words = re.findall(r"\b[a-zA-Z]{4,}\b", text.lower())
top_words = [w for w,_ in Counter(words).most_common(8)]
st.subheader(f"{sel} — Predicted: {r['predicted_rating']} ⭐")
